...
EXPOSE 80
ENTRYPOINT ["java","-Djava.security.egd=file:/dev/urandom","-jar", "aplmu.jar","--spring.data.rest.base-path=${restpath}","--spring.datasource.url=${dburl}","--spring.datasource.username=${dbun}","--spring.datasource.password=${dbpd}"]